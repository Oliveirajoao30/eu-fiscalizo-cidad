...
const Cadastro = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (signupError) {
      setError("Falha ao cadastrar. E-mail já utilizado ou senha inválida.");
    } else {
      navigate("/login");
    }
  };
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-white">
      ...
      >
        <h2 className="text-2xl font-bold text-black mb-2">Cadastro</h2>
        <div>
        ...
        <div className="text-sm text-black text-center mt-2">
          Já tem conta?{" "}
          <a href="/login" className="underline text-black">Entrar</a>
        </div>
      </form>
    </div>
  );
